...
- **`sort NAME [ascending|descending]`** — statement; sorts a `list of number` or `list of string` **in place**. Default direction is ascending. Stable. `NAME` must resolve to a list reference (compile error if it's a dictionary or non-list type). Does NOT update `it`.
...
- **Type aliases — `type NAME is TYPEEXPR`.** Verbose types like `dictionary from Point to number` and `list of Row` repeat across function signatures and bindings. Aliases let users write `type PointCache is dictionary from Point to number` once and use `PointCache` anywhere a type annotation is legal. Implementation: lexer adds `type` keyword (currently unreserved); parser adds top-level `TypeAliasDeclaration` (name + parsed `TypeAnnotation`); the type-resolution layer (`resolveType` / wherever `ChatterType` is materialized from a `TypeAnnotation`) gets a lookup step that expands aliases before producing the canonical `ChatterType`. Aliases live at module top-level alongside functions/structs and follow the same scoping/import rules: `export type` makes an alias importable; `use NAME from "..."` works for aliases just like functions/structs. Forward references within a module are allowed (parallel to forward struct refs). Cycle detection: `type A is B / type B is A` → compile error `circular type alias: A → B → A` (DFS). Aliases are pure compile-time sugar — no runtime representation, no bytecode changes, error messages should still print the expanded form (e.g. "expected dictionary from Point to number, got list of number") rather than the alias name to avoid confusion when the same underlying type has multiple aliases.
- **Multiline function headers / call sites / make / collection literals.** Function declarations like `function get_universes takes list of Row rows from Point point with dictionary from Point to number cache returns number is` get unreadable. The trailing `is` is an unambiguous terminator, so we can permit NEWLINE/INDENT/DEDENT tokens to be skipped while parsing the function header. Implementation: introduce a "soft layout" mode in the parser; toggle on when `function` is consumed (or when the function-header production starts), toggle off when `is` is consumed; while the mode is on, `peek()` / `advance()` silently skip NEWLINE/INDENT/DEDENT tokens. Single-line headers continue to work unchanged (no NEWLINEs to skip). Same trick applies (separately) to: (a) **call sites** — long named-arg calls can break before each separator label, terminated by the implicit "next statement" signal (NEWLINE at base indent); (b) **`make X with FIELD VALUE, ...`** constructions — break before each comma; (c) **list / unique-list / dictionary literals** — break before each comma. Call sites are the trickiest because the terminator is implicit rather than a keyword like `is`; the rule is probably "skip NEWLINE/INDENT only between a named-arg label and its value, or between a value and the next label-or-comma". Trailing-comma tolerance in params, make fields, and literals is a small sibling change worth bundling. Pairs naturally with type aliases: aliases shorten signatures so the multiline form is reserved for genuinely long cases. Recommendation: do type aliases first (root cause), then multiline as the safety valve.
